...
const GameApp_1 = require("./renderer/pixi/GameApp");
...
async function main() {
    ...
    // 生成地图
    const map = new MapGenerator_1.GameMap(CONFIG.map.width, CONFIG.map.height, CONFIG.map.seed);
    map.generate();
    console.log('✅ 地图生成完成');
    // 创建游戏应用
    try {
        game = new GameApp_1.GameApp(map, CONFIG.viewport.width, CONFIG.viewport.height);
        await game.init();
    ...
    // 屏幕坐标转世界坐标
    function screenToWorld(screenX, screenY, cam) {
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.key === '1')
            camera.setZoom('FAR');
        if (e.key === '2')
            camera.setZoom('NORMAL');
        if (e.key === '3')
            camera.setZoom('CLOSE');
        // WASD / 方向键移动
