...
const DNA_1 = require("../systems/dna/DNA");
const TILE_SIZE = 64;
class Character {
    constructor(type, x, y, name, canMove) {
        // 状态 - 营养系统
        // 初始100% = 每日需求的1/3（约1顿饭的量）
        this.calories = 100; // 当前饱食度 0-100%
        this.water = 100; // 当前口渴度 0-100%
        this.energy = 5; // 精力 5=满, 0=空
        this.health = 100; // 生命值 0-100
        // 经验系统（受智力影响）
        this.experience = 0; // 当前经验
        this.level = 1; // 当前等级
        this.skillExp = new Map(); // 技能经验
        // 当前行动
        this.action = '闲置';
        this.target = null;
        // 手中物品
        this.heldItem = null; // 物品类型
        this.heldItemCount = 0; // 物品数量
        // LLM控制
        this.useLLM = false; // 是否使用LLM决策
        this.lastLLMDecision = ''; // 上次LLM决策
        // 死亡原因
        this.deathCause = '';
        // 死亡时间（游戏时间戳）
        this.deathTime = 0;
        ...
        this.dna = DNA_1.DNA.createInitial();
    ...
    getPersonality() {
        ...
        // 判断性格类型
        if (bravery > 0.7 && fear > 0.7 && aggr > 0.7)
            return '勇士';
        if (bravery > 0.7 && fear < 0.4 && aggr > 0.7)
            return '莽夫';
        if (bravery > 0.7 && fear > 0.7 && aggr < 0.4)
            return '谨慎战士';
        if (bravery < 0.4 && fear < 0.4 && aggr < 0.4)
            return '逃兵';
        if (bravery < 0.4 && fear > 0.7 && aggr < 0.4)
            return '旁观者';
        if (aggr > 0.7)
            return '挑衅者';
        if (fear < 0.4)
