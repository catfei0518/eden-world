...
exports.ItemLayer = exports.GameItem = void 0;
...
class ItemLayer {
    constructor(map) {
        ...
        this.currentSeason = 'summer'; // 默认夏天
        // 点击回调
    ...
    generateSeasonalItems() {
        ...
        for (let y = 0; y < mapSize.height; y++) {
            for (let x = 0; x < mapSize.width; x++) {
                ...
                // 草地上5%概率生成花朵
                if (tile.type === MapGenerator_1.TileType.GRASS && rand < 0.05) {
                ...
                // 平地上2%概率生成花朵
                else if (tile.type === MapGenerator_1.TileType.PLAIN && rand < 0.02) {
                    const item = new GameItem('flower', x, y, 'ground');
                    this.items.push(item);
                    this.createSprite(item);
                    count++;
            ...
            for (let x = 0; x < mapSize.width; x++) {
                ...
                switch (tile.type) {
                    ...
                    case MapGenerator_1.TileType.PLAIN:
                        if (rand < 0.15)
                            this.items.push(new GameItem('stick', x, y, 'ground'));
                        else if (rand < 0.26) {
                            const item = new GameItem('branch', x, y, 'ground');
                    ...
                    case MapGenerator_1.TileType.HILL:
                        if (rand < 0.08)
                            this.items.push(new GameItem('rock', x, y, 'ground'));
                        else if (rand < 0.15) {
                            const item = new GameItem('bush', x, y, 'low');
    ...
    async loadTextures() {
        ...
        for (const type of Object.keys(this.ASSETS)) {
            for (const season of seasons) {
                const path = this.ASSETS[type][season];
