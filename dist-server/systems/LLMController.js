...
class LLMController {
    constructor() {
        this.ollamaUrl = '/api/llm';
        this.model = 'qwen3.5:2b';
        this.characters = new Map();
        this.lastDecision = new Map();
        this.decisionInterval = 3000; // 3秒决策一次
        // 可用动作
        this.actions = [
            '寻找食物', '寻找水源', '吃东西', '喝水',
            '休息', '探索', '闲置'
    ...
    buildPrompt(char, world) {
        ...
        if (foods.length > 0) {
            ...
            const dist = Math.sqrt(Math.pow(nearest.position.x - char.x, 2) +
                Math.pow(nearest.position.y - char.y, 2)).toFixed(1);
...
{"action":"动作名"}`;
    ...
    goToNearest(char, items) {
        ...
        for (const item of items) {
            const dist = Math.sqrt(Math.pow(item.position.x - char.x, 2) +
