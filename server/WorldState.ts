...
export class WorldState {
    ...
    private regionNoise: NoiseFunction2D;  // 大区域噪声
    private detailNoise: NoiseFunction2D;  // 细节噪声
    ...
    constructor(seed: number = WORLD_SEED) {
        ...
        this.itemNoise = createNoise2D(itemRNG);
    ...
    private generateWaterBodies(): void {
        ...
        for (let y = 0; y < MAP_HEIGHT; y++) {
            for (let x = 0; x < MAP_WIDTH; x++) {
                ...
                // 非水体格子，先用占位符
                this.tiles.push({ x, y, type: 'grass' as TerrainType });
            }
        }
    ...
    // 打印 region 噪声值分布
    private printRegionDistribution(): void {
    ...
    // 打印完整地形统计
    private printFullTerrainStats(): void {
        const stats: Record<string, number> = {};
    ...
    // 新的地形类型决定函数 - 多样化区域型
    private getTerrainTypeNew(elevation: number, moisture: number, region: number, detail: number, x: number, y: number): TerrainType {
        // 归一化到0-1
    ...
    private generateGroundObjects(): void {
        ...
        for (let y = 0; y < MAP_HEIGHT; y++) {
            for (let x = 0; x < MAP_WIDTH; x++) {
                ...
                const tileIndex = y * MAP_WIDTH + x;
                const tile = this.tiles[tileIndex];
                if (!tile || tile.x !== x || tile.y !== y) continue;
